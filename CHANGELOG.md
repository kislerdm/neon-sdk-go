...
- **[BREAKING]** Added the attribute `slug` to the method `CreateSnapshot` to configure the snapshot resource ID unique within a project.
- Added the field `Slug` to the type `Snapshot`.

## [v0.27.0] - 2026-09-20

### Added

- Added custom deserialization of the types to address polymorphism:
...
**Note** that the versions v0.26.0 was _retracted_.

## [v0.26.0] - 2026-09-16

### Fixed

- Fixed the methods `CreateProjectBranchTrigger`, `UpdateProjectBranchTrigger` and `UpdateNeonAuthEmailProvider`.

### Added

- Added custom serialization of the types to address polymorphism:
  - `NeonAuthEmailServerConfig`;
  - `NeonAuthEmailServerConfigResponse`;
  - `Trigger`;
  - `TriggerCreateRequest`;
...
**Note** that the versions v0.24.0, v0.25.0 were _retracted_.
...
- **[BREAKING]** Fixed the definition of the following methods:
  - `DeleteProjectBranch`;
...
The release incorporates the up-to-date [API contract](openAPIDefinition.json) as of 2026-09-07 00:02:00 GMT.

### Fixed

- Fixed serde of enums.
...
- **[BREAKING]** Changed the type of the attribute `PgSettings` to `map[string]any`. 
  It affects the types `EndpointSettingsData` and `DefaultEndpointSettings`.

## Removed

- **[BREAKING]** Removed the type `PgSettingsData`.

## [v0.15.0] - 2025-10-26

The release incorporates the up-to-date [API contract](openAPIDefinition.json) as of 2025-10-26 18:52:00 GMT.

### Added

- Added the attribute `RestoredActions` to the type `Branch`.
